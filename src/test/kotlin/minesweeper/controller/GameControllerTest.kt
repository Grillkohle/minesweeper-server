...
class GameControllerTest(
        ...
        mockMvc.perform(
            ...
            .andExpect(
                MockMvcResultMatchers.header()
                    .string(HttpHeaders.LOCATION, containsString("/games/${expectedResponse.id}"))
            )
    }
    ...
    fun `change cell state resource not found expect 404`() {
        ...
        ).thenThrow(ResourceNotFoundException("Failed to find resource."))
    ...
    @Test
    fun `change cell state game already lost expect 422`() {
        val gameId = UUID.randomUUID()
        val request = CellStateTransitionRequest(
            horizontalIndex = 0,
            verticalIndex = 0,
